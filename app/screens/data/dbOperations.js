...
const restartInteractionForUser = async (
  chatInteractionId,
  userWhoRestartsInteraction,
  UserToRestartInteractionWith,
  adId
) => {
  try {
    const db = getDatabase(app);
    const userChatInteractionRef = ref(
      db,
      `users/${UserToRestartInteractionWith}/chatInteraction/${chatInteractionId}/`
    );
    const chatInteractionRef = ref(db, `chatInteraction/${chatInteractionId}/`);
    ...
    const updatedData = [
      ...previousData,
      { message: message, senderId: userWhoRestartsInteraction },
    ];
...
const sendMessage = async (adId, senderId, message) => {
  try {
    ...
    const userChatInteractionRef = ref(
      db,
      `users/${senderId}/chatInteraction/`
    );
    ...
    for (const chatInteractionId of chatInteractionIds) {
      ...
      if (item.adId === adId) {
        ...
        const chatInteractionRef = ref(
          db,
          `chatInteraction/${chatInteractionId}/`
        );
        const chatInteractionResponse = await get(chatInteractionRef);
        const previousData = chatInteractionResponse.val() || [];
        const updatedData = [
    ...
    if (!existsInChatInteraction) {
      ...
      const response = await push(chatInteractionRef, [
        { sellerId: ownerId, buyerId: senderId },
        { message: message, senderId: senderId },
      ]);
      const chatInteractionId = response.key;
      const userChatInteractionRef = ref(
        db,
        `users/${senderId}/chatInteraction/${chatInteractionId}/`
      );
      const ownerChatInteractionRef = ref(
        db,
        `users/${ownerId}/chatInteraction/${chatInteractionId}/`
      );
      await set(userChatInteractionRef, { adId: adId });
...
const deleteAdInteractions = async (userId, adIds) => {
  try {
    ...
    const result = adIds.map(async (adId) => {
      ...
      const userChatInteractionRef = ref(
        db,
        `users/${userId}/chatInteraction/${adInteractionId}`
      );
